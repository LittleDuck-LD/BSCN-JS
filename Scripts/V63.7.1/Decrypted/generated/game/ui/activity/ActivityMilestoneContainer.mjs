...
import { getSCFile, getCollabEvent, getPlayerCollabEventData, getScoreForCompleteAllMilestones } from "./activity-utils.mjs";
...
class ActivityMilestoneContainer extends GUIContainerBase {
  ...
  setupEventSkins() {
    ...
    for (let i = 0; i < skinAlbum.getSkinsCount(); i++) {
      ...
      if (!skin || skinAlbum.isExcludedFromCompletionCheck(skin))
        continue;
      this.heroAreasChangeIdle?.push(false);
      ++skinOfferNumber;
  ...
  setupEventMilestone() {
    ...
    let collabEvent = getCollabEvent();
    let playerCollabEventData = getPlayerCollabEventData();
    ...
    let maxCollabScore = getScoreForCompleteAllMilestones();
      ...
      if (!SC.LogicDataTables.getClientGlobals().getBoolValueSafe("DISABLE_UNLOCK_BRAWLER_INFO_POPUP_HACK", false)) {
        for (let wrapperChildIndex = 0; wrapperChildIndex < clip.getTimelineChildCount(); ++wrapperChildIndex) {
          ...
          if (!SC.MovieClipHelper.getMovieClipOptional(wrapper, "brawler_unlock"))
            continue;
          if (!SC.MovieClipHelper.hasMovieClip(getSCFile(), "icon_collab_activity"))
            break;
          let replacementInfoImage = SC.StringTable.getMovieClip(getSCFile(), "icon_collab_activity");
  ...
  openSkinPreview(skin) {
    ...
    let wingType = skin?.getConfData()?.m_pSkinConfigDataK2?.getWingType_k2();
    if (wingType != void 0 && wingType > 0) {
      popup?.getPopup()?.setText("TID_SHOP_SPECIAL_OFFER", SC.StringTable.getString("TID_CATALOG_COLLECTION_SKINS_BACK"));
      popup?.getMovieClip()?.setText("info_subtitle", SC.StringTable.getString("TID_CATALOG_COLLECTION_SKINS_BACK"));
      let contentTf = popup?.getMovieClip()?.getTextFieldByName("info_txt");
      if (contentTf) {
        contentTf.setText(contentTf.getText().replace(SC.StringTable.getString("TID_SKINS_TITLE"), SC.StringTable.getString("TID_CATALOG_COLLECTION_SKINS_BACK")));
      }
    }
    return popup;
  }
  getMilestoneRewardState(threshold) {
    ...
    let playerCollabData = getPlayerCollabEventData();
  ...
  // changed to idle
  onUpdate(dt) {
    if (!this.heroAreasChangeIdle) return;
    try {
      let idle = SC.SceneCharacter.AnimId.ANIM_IDLE;
      let eventSkins = this.contentMovieClip;
      let allChanged = true;
      let len = this.heroAreasChangeIdle.length;
      for (let i = 0; i < len; i++) {
        if (!this.heroAreasChangeIdle[i]) {
          let skinDisplay = SC.MovieClipHelper.getMovieClipOrWarning(eventSkins, "item_" + (i + 1));
          if (skinDisplay != null) {
            let heroArea = SC.MovieClipHelper.getMovieClipOrWarning(skinDisplay, "unlock", "hero_area");
            let count = heroArea.getNumChildren();
            for (let j = 0; j < count; j++) {
              let c = heroArea.getChildAt(j);
              if (c.isSprite() && !c.isMovieClip() && !c.isTextField() && !c.isCustomButton() && !c.isShape()) {
                c.as(SC.SceneRenderer).changeAnimationTo(idle, idle);
                this.heroAreasChangeIdle[i] = true;
              }
            }
          }
          if (!this.heroAreasChangeIdle[i])
      ...
      if (allChanged) this.heroAreasChangeIdle = null;
    } catch (e) {
    }
  }
  onDestruct() {
