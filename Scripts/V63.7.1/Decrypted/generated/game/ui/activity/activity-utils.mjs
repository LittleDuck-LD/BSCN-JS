...
function getSCFile() {
  ...
  if (getModule(ActivityModule).isK2CollabEvent) {
    return SC.CollabUtil.getK2DownloadedScFilenameAny();
  } else {
...
function getCollabData() {
  if (getModule(ActivityModule).isK2CollabEvent) {
    return SC.CollabUtil.getK2CollabData();
  } else {
    return SC.CollabUtil.getCollabData();
  }
}
function getCollabEvent() {
  if (getModule(ActivityModule).isK2CollabEvent) {
    return SC.CollabUtil.getK2CollabEvent();
  } else {
    return SC.CollabUtil.getCollabEvent();
  }
}
function getPlayerCollabEventData() {
  if (getModule(ActivityModule).isK2CollabEvent) {
    return SC.CollabUtil.getK2PlayerCollabEventData();
  } else {
    return SC.CollabUtil.getPlayerCollabEventData();
  }
}
function getScoreForCompleteAllMilestones() {
  return SC.CollabUtil.getK2ScoreForCompleteAllMilestones();
}
...
export {
  ...
  getCollabEvent,
  getLotteryDsc,
  getPlayerCollabEventData,
  getResourceCount,
  getSCFile,
  getScoreForCompleteAllMilestones,
  isLocalDebug,
