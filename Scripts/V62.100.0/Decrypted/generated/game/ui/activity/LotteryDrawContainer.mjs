...
import { addCommand, getString } from "../../base/util.mjs";
...
class LotteryDrawContainer extends GUIContainerBase {
  constructor(container) {
    ...
    try {
      ...
      this.subscribe("cpp_K2RandomChestsRewardRecordMessage", this.onRewardRecordMessage);
  ...
  onRewardRecordMessage(evtData) {
    ...
    this.showRewardRecords(rewardRecordMessage);
  }
  ...
  buyItemAndContinueLottery(resourceName, price, times, btn, notEnoughBillingPackage, PurchaseOption, hideTokenPopup = false) {
  ...
  tryDoLottery(times, chestType, resourceConsume, notEnoughBillingPackageNamePrefix, drawOption = SC.LogicK2DrawCommand.DrawOption.DrawOption_K2_DRAW_TOKEN) {
    ...
    if (ResultEnum.VALIDATION_RESULT_OK == canExecute) {
      ...
      openGenericPopup({ module: "ui/activity/CommonPurchasePopup.mjs", scFile: getSCFile(), clip: "popup_generic", para: [popupConfig], darkenBackground: true });
    } else {
