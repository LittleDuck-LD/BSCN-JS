...
import { waitEvent, waitPopup } from "../base/util.mjs";
import { AbortableController } from "../base/AbortableController.mjs";
import { ActivityModule } from "../module/ActivityModule.mjs";
function initDebugMenuK2(debugMenu) {
  ...
  debugMenu.addButton("TypeScript", "K2Perf", 0, () => {
    ...
    SC.GUI.getInstance().hudPrint("Hello from typescript " + getModule(ActivityModule).maybeAtBattle, false, false, false);
  }, 0);
  debugMenu.addButton("js-waitevent", "K2Perf", 0, async () => {
    let a = await waitEvent("cpp_popup_open");
    console.log("wait event ", a);
  }, 0);
  let ac = new AbortableController();
  debugMenu.addButton("js-waitevent-ac", "K2Perf", 0, async () => {
    try {
      let a = await waitEvent("cpp_popup_open", 5e3, ac.signal);
      console.log("wait event waitevent ", a);
    } catch (e) {
      console.log("wait event waitevent error", e);
      ac = new AbortableController();
    }
  }, 0);
  debugMenu.addButton("js-waitevent-abort", "K2Perf", 0, async () => {
    ac.abort("click to waitevent");
  }, 0);
  let ac_panel = new AbortableController();
  debugMenu.addButton("js-waitpanel", "K2Perf", 0, async () => {
    try {
      let a = await waitPopup(SC.PopupBase.PopupType.HERO_LIST_POPUP);
      console.log("waitpanel HERO_POPUP");
    } catch (e) {
      console.log("wait event waitpanel error", e);
      ac_panel = new AbortableController();
    }
  }, 0);
  debugMenu.addButton("js-waitpanel-ac", "K2Perf", 0, async () => {
    try {
      await waitPopup(SC.PopupBase.PopupType.HERO_LIST_POPUP, 5e3, ac_panel.signal);
      console.log("wait event waitpanel ");
    } catch (e) {
      console.log("wait event waitpanel error", e);
      ac_panel = new AbortableController();
    }
  }, 0);
  debugMenu.addButton("js-waitpanel-abort", "K2Perf", 0, async () => {
    ac_panel.abort("click to waitpanel");
  }, 0);
  console.log("DebugMenu init in typescript");
